...
CONTAINS
  ...
  SUBROUTINE new_item (label, linenumber, spos, luppercase, item)

    ! Initialize new item.

    ! Dummy arguments:
    INTEGER, INTENT(in) :: label, linenumber, spos
    LOGICAL, INTENT(in) :: luppercase
  ...
  SUBROUTINE list
    ...
    ELSE                               ! List contains at least one item
       ...
       DO
          ...
          PRINT *, ptr%label, ptr%linenumber, ptr%spos
...
END MODULE m_linked_list_do_loops
